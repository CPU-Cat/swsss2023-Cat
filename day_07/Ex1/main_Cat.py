...
import runge_kutta_Cat as rk

def dormand_prince_integrator(f, x, t, h):
    a = dp.a
    b = dp.b
    c = dp.c
    step = rk.explicit_RK_stepper(f,x,t,h,a,b,c)
    return step # please complete this function
...
ax.set_xlabel("time")
ax.set_ylabel("x(t)")
ax.plot(time_points, trajectory, linewidth=2, color="red", marker = "o", label = 'RK solve')
ax.plot(time_points_analytical, trajectory_analytical, linewidth=2, color="black", linestyle="dashed", label = 'analytical')
plt.legend()
plt.show()